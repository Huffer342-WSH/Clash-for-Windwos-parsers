module.exports.parse = async (raw, { axios, yaml, notify, console }, { name, url, interval, selected }) => {
    ...
    const proxiesDefault = ['默认代理', 'DIRECT', ...proxiesUseful];

    const proxiesChatgpt = proxiesUseful.filter(proxy => !proxy.includes('香港') && !proxy.includes('套餐') && !proxy.includes('网址') && !proxy.includes('客服'));

    const proxiesCountry = [];
